...
# TODO 聚类为1的C2C,并且是有向的，然后这个成为邻接矩阵，既后面的pos_index
# TODO 结点特征为正确率-错误率（总体认知情况）、学生群体k1的认知、群体k2的认知、...、难度
# x为特征矩阵,使用我采集的特征 然后emb
# TODO hidden_emb = mu.cpu().data.numpy()
# TODO 训练结束后再clustering
...
def gae_train(epoc_trainh, x, model, optimizer, train_pos_edge_index):
    model.train()
    optimizer.zero_grad()
...
def train_skill_emb(model, data, skill_df):
    ...
    for epoch in tqdm(range(1, 1001)):   # 1001 轮
        try:
            gae_train(epoch, x, model, optimizer, train_pos_edge_index)  # # NOTE 在这里不需要传入self
    ...
    return skill_emb, model.encoder.feature.cpu()  # NOTE 返回学习的嵌入的字典啊
...
if __name__ == '__main__':
    ...
    # NOTE 训练知识概念群c2c
    edge_index = get_skill_group_logic(df, skill_df, skill_group_threshold, train_skill_group_logic=False)
    ...
    GAEmodel = pyg_nn.GAE(Encoder(x.shape[1], channels_dim)).to(dev)  # 设为x的维度
    skill_emb, model_skill_emb = train_skill_emb(model=GAEmodel, data=data, skill_df=skill_df)  # NOTE 在这里不需要传入self
    ...
    #  复制skill_df 然后插入
    skill_group = pd.DataFrame(skill_emb).transpose().iloc[:,:0]
    skill_group.insert(loc=0, column="skill_group", value=skill_group_byCluster)
